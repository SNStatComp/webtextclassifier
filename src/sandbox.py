...
from LLM import classify, create_model, embed_text
...
def main():
    ...
    for var in variables:
        ...
        for i, uc_tuple in enumerate(url_country):
            ...
            embedding = embed_text(models["embed"], extracted_text, var)
            embeddings[var].append(embedding)
    ...
    print("Done!")
    print(embeddings)
    np.save("embeddings_pickly.npy", embeddings)

    first_embedding = embeddings[list(embeddings.keys())[0]][0]

    for key in embeddings.keys():
        embeddings_key = embeddings[key]
        plot_similarity_distribution(first_embedding, embeddings_key, key)


def plot_similarity_distribution(first_embedding, embeddings, title):
    similarities = cosine_similarity([first_embedding], embeddings)
